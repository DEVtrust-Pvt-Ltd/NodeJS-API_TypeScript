...
import { trimUrl, getFormattedDatehms, getFormattedDate } from "../../utility/commonMethod";
...
export const createUserJoureny = async (
  ...
  const baseFileName = `User Journey Description-${trimUrl(domain.domainName)}-${formattedDate}`;
...
export const updateUserJoureny = async (
  ...
  //if (domain.userJourneyDocId) {
  //await updateGoogleDoc(domain.userJourneyDocId, userJourney);
  //await Domain.update(id, { userJourney });
  //} else {
  const date = getFormattedDatehms();
